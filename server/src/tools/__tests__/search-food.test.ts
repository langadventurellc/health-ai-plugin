...
const USDA_RESULTS: FoodSearchResult[] = [
  { id: "171705", source: "usda", name: "Chicken, breast, skinless, boneless", brand: null, matchScore: 247 },
  { id: "171706", source: "usda", name: "Chicken, thigh, skinless", brand: null, matchScore: 200 },
  { id: "171707", source: "usda", name: "Brown Rice", brand: null, matchScore: 150 },
];

const OFF_RESULTS: FoodSearchResult[] = [
  { id: "3017620422003", source: "openfoodfacts", name: "Chicken Breast Skinless Boneless", brand: "FoodCo", matchScore: 0.95 },
  { id: "3017620422004", source: "openfoodfacts", name: "Organic Quinoa", brand: "NatureBrand", matchScore: 0.9 },
  { id: "3017620422005", source: "openfoodfacts", name: "Brown Rice", brand: "RiceCo", matchScore: 0.85 },
];
...
describe("isDuplicate", () => {
  it("identifies cross-source duplicates by substring match", () => {
    const usda: FoodSearchResult = {
      id: "1", source: "usda", name: "Chicken Breast", brand: null, matchScore: 100,
    };
    const off: FoodSearchResult = {
      id: "2", source: "openfoodfacts", name: "Chicken Breast Skinless", brand: null, matchScore: 0.9,
    };
  ...
  it("does not flag items from the same source", () => {
    ...
    const b: FoodSearchResult = {
      id: "2", source: "usda", name: "Chicken Breast", brand: null, matchScore: 90,
    };
  ...
  it("does not flag clearly different items", () => {
    ...
    const off: FoodSearchResult = {
      id: "2", source: "openfoodfacts", name: "Organic Quinoa", brand: null, matchScore: 0.9,
    };
...
describe("deduplicateResults", () => {
  ...
  it("returns all items when there are no duplicates", () => {
    ...
    const off: FoodSearchResult[] = [
      { id: "2", source: "openfoodfacts", name: "Banana", brand: null, matchScore: 0.9 },
    ];
...
describe("handleSearchFood", () => {
  it("returns results from a single source when specified", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({ foods: [{ fdcId: 1, description: "Apple", score: 100 }], totalHits: 1 }),
  ...
  it("returns results from the working source and includes a warning when the other fails", async () => {
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      return new Response(
        JSON.stringify({
          count: 1,
          products: [{ code: "123", product_name: "Apple Juice", brands: "JuiceCo" }],
        }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    });
    ...
    expect(response.results.some((r) => r.source === "openfoodfacts")).toBe(true);
  ...
  it("returns cached combined results on subsequent calls", async () => {
    const cachedResults: FoodSearchResult[] = [
      { id: "1", source: "usda", name: "Cached food", brand: null, matchScore: 100 },
    ];
