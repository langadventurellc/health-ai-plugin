...
export class OpenFoodFactsClient {
  ...
  async searchFoods(
    ...
    const cached = this.cache.getSearchResults('openfoodfacts', query) as
      | FoodSearchResult[]
      | null;
    if (cached) {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `Open Food Facts search API error: ${response.status} ${response.statusText}`,
        );
  ...
  async getNutrition(
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `Open Food Facts product API error: ${response.status} ${response.statusText}`,
        );
  ...
  private fallbackSearchResults(
    ...
    const stale = this.cache.getSearchResultsStale('openfoodfacts', query) as
      | FoodSearchResult[]
      | null;
